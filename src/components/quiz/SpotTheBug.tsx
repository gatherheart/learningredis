...
export function SpotTheBug({ lessonId, quizId, question, code, buggyLine, onSolved }: Props) {
  const { t } = useTranslation();
  const alreadyDone = isQuizCompleted(lessonId, quizId);
  const [selected, setSelected] = useState<number | null>(alreadyDone ? buggyLine : null);
        ...
        {lines.map((line, index) => {
          ...
          return (
            <button
              ...
              className={`flex w-full px-3 py-0.5 text-left ${
                isSelected ? "bg-emerald-500/12 ring-1 ring-emerald-400/60" : "hover:bg-white/[0.06]"
