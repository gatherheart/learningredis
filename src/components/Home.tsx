...
export function Home({ lessons }: Props) {
  ...
  const nextLesson = lessons.find((lesson) => !lessonProgress(lesson.id, lesson.quizzes.length).done) ?? lessons[0];

  return (
    <main className="app-scrollbar flex-1 overflow-y-auto bg-[radial-gradient(circle_at_top_right,rgba(52,211,153,0.16),transparent_28%),linear-gradient(180deg,rgba(6,10,15,0.92),rgba(4,8,12,0.98))]">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-5 lg:px-6 lg:py-6">
        <section className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(360px,0.85fr)]">
          ...
          <div className="rounded-[30px] border border-white/10 bg-[#0a121a] p-5 shadow-2xl shadow-black/20">
            <div className="mb-4 flex items-center justify-between">
              <div className="text-xs uppercase tracking-[0.16em] text-zinc-500">preview</div>
        ...
        <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
          <div className="rounded-[28px] border border-white/10 bg-zinc-950/80 p-5">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-zinc-50">Mission map</h2>
              <div className="text-xs uppercase tracking-[0.14em] text-zinc-500">path</div>
            </div>
              ...
              {lessons.map((lesson, index) => {
                ...
                return (
                  <Link
                    key={lesson.id}
                    to={`/lesson/${lesson.id}`}
                    className="rounded-[22px] border border-white/10 bg-white/[0.04] p-4 transition hover:border-emerald-400/35 hover:bg-emerald-500/8"
                  >
                    <div className="mb-2 flex items-center justify-between">
                      <span className="font-mono text-xs text-zinc-500">{String(index + 1).padStart(2, "0")}</span>
                      <span className="text-[10px] uppercase tracking-[0.22em] text-zinc-500">{done ? "done" : lesson.difficulty}</span>
                    </div>
                    <div className="text-sm font-medium text-zinc-100">{lesson.title}</div>
                    <div className="mt-2 text-xs uppercase tracking-[0.18em] text-zinc-500">{lesson.topic}</div>
                  </Link>
                );
              })}
            </div>
          ...
          <div className="rounded-[28px] border border-white/10 bg-zinc-950/80 p-5">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-zinc-50">How this course works</h2>
              <div className="text-xs uppercase tracking-[0.14em] text-zinc-500">flow</div>
            </div>
            <div className="space-y-3">
              {[
                "Edit the Redis command script inside each lesson.",
                "Run the mission and watch keys, TTLs, queues, or streams animate after every command.",
                "Clear the lesson quiz set to unlock the next stage and move into harder operational scenarios.",
              ].map((item, index) => (
                <div key={item} className="flex gap-4 rounded-[22px] border border-white/10 bg-white/[0.04] p-4">
                  <div className="grid h-9 w-9 shrink-0 place-items-center rounded-2xl bg-emerald-500/10 font-mono text-sm text-emerald-200">
                    {index + 1}
                  </div>
